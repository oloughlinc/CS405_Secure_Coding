...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
  ...
  { 
    ...
    output.at(i) = source.at(i) ^ key.at(i % key_length);
  }
...
std::string read_file(const std::string& filename)
{
  ...
  inFS.open(filename);
  assert(inFS.is_open());
  strStream << inFS.rdbuf(); // pass entire buffer to a stringstream
  file_text = strStream.str(); // convert to std string
  inFS.close();
...
std::string getDate() {
    ...
    time_t rawTime;
    struct tm dateTime;
    time(&rawTime); // get raw time, epoch
    localtime_s(&dateTime, &rawTime); // convert to local date. localtime_s is MS thread-safe version 

    // build string
    std::stringstream date;
    // https://cplusplus.com/reference/ctime/tm/
    date << (1900 + dateTime.tm_year) << '-' << (1 + dateTime.tm_mon) << '-' << dateTime.tm_mday;
    return date.str();
}
