...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        /*
        *  Before performing the actual addition operation and changing the value of result,
        * first check that the operation would not exceed the maximum value of this datatype
        * if so, the actual operation is not performed and an error is thrown
        */

        // if result is already above the datatype max value less one increment, as in, another addition of increment will overflow
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    {
        /*
        *  Before performing the actual subtraction operation and changing the value of result,
        * first check that the operation would not result in a value less than zero
        * if so, the actual operation is not performed and an error is thrown
        */

        //if (result < (std::numeric_limits<T>::lowest() + decrement)) { // true datatype underflow, going below datatype min
        if (result < decrement) { // underflow defined as a negative number as a result, ex in a case where this number could be used as an array index
            throw std::overflow_error("UNDERFLOW ERROR");
        }
        else result -= decrement;
    }
...
void test_overflow()
{
    ...
    /*
    * Added exception handling in the case that the addition/subtraction functions report an overflow error.
    * The error is reported to the screen instead of the value. In that case, the result is invalid and not acquired.
    */
