...
#include <tuple>
#include <vector>
#include <cctype> // recognizing whitespace characters
...
#define BLACKLIST_OK 0
#define BLACKLIST_FAIL 1
bool blacklist(std::string sql) {

    /* 
    *  blacklist method to catch a SPECIFIC type of injection, in this case OR TRUE to return all entries
    *  This is not ideal
    *  Does not handle multiple OR statements for example: OR command OR command OR true
    *  better to change the way that sql commands are created, but we are limited in the assignment to just this function
    *  and this function already gets the finalized sql command
    */
    ...
    // if there is no OR statement return OK, else mark the position
    size_t orPosition;
    if ((orPosition = query.find(" or")) != std::string::npos) {}
    else if ((orPosition = query.find("'or")) != std::string::npos) {}
    else if ((orPosition = query.find("\"or")) != std::string::npos) {}
    else { return BLACKLIST_OK; }

    // adjust OR position to end (after 'R')
    orPosition += 4;
    
    // the rest of the input after the OR
    std::string suspectInput = query.substr(orPosition);

    // remove all whitespace: https://www.scaler.com/topics/removing-whitespace-from-a-string-in-cpp/
    suspectInput.erase(std::remove_if(suspectInput.begin(), suspectInput.end(), ::isspace), suspectInput.end());

    // split string by equal sign (return FAIL if none found)
    size_t equalPosition;
    if ((equalPosition = suspectInput.find('=')) == std::string::npos) return BLACKLIST_FAIL;
    std::string LHS = suspectInput.substr(0, equalPosition);
    std::string RHS = suspectInput.substr(equalPosition + 1);

    // if the strings are equal on each side of the '='
    if (LHS.compare(RHS) == 0) { return BLACKLIST_FAIL; }
    else return BLACKLIST_OK;
}
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // added check against blacklist: generic OR TRUE
  if (blacklist(sql)) {
      std::cout << "\nPossible SQL injection detected. Aborted DB command." << std::endl;
      std::cout << "SQL command was: " << std::endl;
